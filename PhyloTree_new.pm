...
sub get_subtree{
    ...
    my $subtree = new PhyloTree_new(-nodes => @subtree_nodes);
...
sub get_lca{
   ...
   for(my $x = 0; $x < scalar(@ancestor_2d); $x++){
	...
	#print Dumper(@ancestor_2d_rev);  ####
	my $longest_row = 0;
	foreach my $row(@ancestor_2d_rev){
		if (scalar(@{ $row }) > $longest_row){
			$longest_row = scalar(@{ $row });
		}		
	}
    my $i = 0;
	print "Searching for lca %%%%%%\n";
	print "Longest row is $longest_row \n";
	print "Number of rows is ".scalar(@ancestor_2d_rev)."\n";
    while(!$lca){
        my $last_id = -1;
        for(my $y = 0; $y < scalar(@ancestor_2d_rev); $y++){
            ...
            if(defined $last_id && $last_id == -1){
                $last_id = $id;
            }else{
                if(defined $id && (!defined $last_id or $id != $last_id)){
                    $lca = $ancestor_2d_rev[$y][$i-1];
                } elsif (!defined $id && defined $last_id && $last_id != -1){
					$lca = $ancestor_2d_rev[$y][$i-1];
				}
				else{
                    $last_id = $id;
        ...
        $i++;
		if ($i > $longest_row){
			#die "Went over the whole array and failed to find lca";
			$lca = $ancestor_2d_rev[scalar(@ancestor_2d_rev)-1][$i-2];
		}
    }
	print "returning lca ===" .($lca-> get_node_id()) . "\n";
    return $lca;
}
